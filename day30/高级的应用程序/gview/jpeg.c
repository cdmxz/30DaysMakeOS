...
unsigned short get_bits(JPEG *jpeg, int bit)
{
	...
	while (remain <= 16)
	{
		...
		c = *jpeg->fp++;
		if (c == 0xff)
		{ // マーカエラーを防ぐため、FF -> FF 00 にエスケープされてる
			if (jpeg->fp >= jpeg->fp1)
			{
				ret = 0;
				goto fin;
...
void jpeg_decode(JPEG *jpeg, UCHAR *rgb, int b_type)
{
	...
	INIT_ZTABLE(0, 0, 1, 8, 16);
	INIT_ZTABLE(4, 9, 2, 3, 10);
	INIT_ZTABLE(8, 17, 24, 32, 25);
	INIT_ZTABLE(12, 18, 11, 4, 5);
	INIT_ZTABLE(16, 12, 19, 26, 33);
	INIT_ZTABLE(20, 40, 48, 41, 34);
	INIT_ZTABLE(24, 27, 20, 13, 6);
	INIT_ZTABLE(28, 7, 14, 21, 28);
	INIT_ZTABLE(32, 35, 42, 49, 56);
	INIT_ZTABLE(36, 57, 50, 43, 36);
	INIT_ZTABLE(40, 29, 22, 15, 23);
	INIT_ZTABLE(44, 30, 37, 44, 51);
	INIT_ZTABLE(48, 58, 59, 52, 45);
	INIT_ZTABLE(52, 38, 31, 39, 46);
	INIT_ZTABLE(56, 53, 60, 61, 54);
	INIT_ZTABLE(60, 47, 55, 62, 63);
