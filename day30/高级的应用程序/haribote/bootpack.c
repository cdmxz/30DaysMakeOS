...
void HariMain(void)
{
	...
	*((int *)0x0fe4) = (int)shtctl;
	...
	/* 载入字库 hzk16.fnt */
	fat = (int *)memman_alloc_4k(memman, 16 * 2880);
	...
	else
	{
		...
		for (i = 0; i < 16 * 256; i++)
		{
			nihongo[i] = hankaku[i]; /*如果没有载入字库，半角部分直接复制英文字库*/
		}
...
struct SHEET *open_console(struct SHTCTL *shtctl, unsigned int memtotal)
{
	...
	unsigned char *buf = (unsigned char *)memman_alloc_4k(memman, width * height);
	sheet_setbuf(sht, buf, width, height, -1); /*无透明色*/
	make_window8(buf, width, height, "console", 0);
	make_textbox8(sht, 8, 28, 484, 263, COL8_000000);
...
void close_console(struct SHEET *sht)
{
	...
	memman_free_4k(memman, (int)sht->buf, 500 * 300);
	sheet_free(sht);
	close_constask(task);
	return;
}
