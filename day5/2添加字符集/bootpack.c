...
void showfont8(char *video_ram, int screen_width, int x, int y, char col8, char *font);
void showfont8_ascstr(char *vram, int xsize, int x, int y, char c, unsigned char *s);
...
void HariMain(void)
{
    ...
    init_palette();
    init_screen(binfo->video_ram, binfo->screen_width, binfo->screen_height);
    showfont8_ascstr(binfo->video_ram, binfo->screen_width, 8, 8, COL8_FFFFFF, "ABC 123");
    showfont8_ascstr(binfo->video_ram, binfo->screen_width, 31, 31, COL8_000000, "Haribote OS.");
    sprintf(s, "scrnx = %d", binfo->screen_width);
    showfont8_ascstr(binfo->video_ram, binfo->screen_width, 16, 64, COL8_FFFFFF, s);
    while (1)
    {
        io_hlt();
    }
...
void showfont8_ascstr(char *vram, int xsize, int x, int y, char c, unsigned char *s)
{
    ...
    for (; *s != '\0'; s++)
    {
        showfont8(vram, xsize, x, y, c, hankaku + *s * 16);
        x += 8;
    }
    return;
}
